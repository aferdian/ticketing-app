<nav class="sticky top-0 z-50 flex justify-between w-full bg-white dark:bg-gray-900 py-4 px-6 shadow-sm">
    ...
    <div class="flex items-center gap-4">
        ...
        @if (request()->routeIs(['home', 'admin.index', 'events.show', 'checkout.form', 'orders.customers']))
            <a href="{{ route('home') }}"
        ...
        <div class="relative">
            ...
            <div id="user-menu"
                ...
                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                    ...
                    @if (Auth::user()->role !== 'superadmin' && Auth::user()->organizations->count() > 0 && session()->has('selected_organization_id'))
                        <p class="text-xs text-gray-500 truncate dark:text-gray-400">
                            {{ Auth::user()->organizations->where('id', session('selected_organization_id'))->first()->name ?? 'N/A' }}
                        </p>
                    @endif
                </div>
                ...
                @if (Auth::user()->role !== 'superadmin' && Auth::user()->organizations->count() > 1)
                    <div class="border-b border-gray-100 dark:border-gray-700 py-2">
                        ...
                        @foreach (Auth::user()->organizations as $organization)
                            <form action="{{ route('organization.change') }}" method="POST">
                                @csrf
                                <input type="hidden" name="organization_id" value="{{ $organization->id }}">
                                <button type="submit"
                                    class="flex items-center gap-2 px-4 py-2 w-full text-left text-gray-900 dark:text-gray-100 hover:bg-gray-600 dark:hover:bg-gray-500/30 transition cursor-pointer">
                                    <i class="ri-building-line"></i> {{ $organization->name }}
                                    @if (session('selected_organization_id') == $organization->id)
